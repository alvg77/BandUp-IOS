...
struct CountryCodes: View {
    ...
    var body: some View {
        NavigationStack {
            List(filteredCountries.sorted(by: <), id: \.key) { key, value in
                HStack {
    ...
    var filteredCountries: [(key: String, value: String)] {
        if search.isEmpty {
            return countryDictionary.map { $0 }
        } else {
            return countryDictionary.filter {
                countryName(countryCode: $0.key)?.lowercased().contains(search.lowercased()) ?? false
                    || $0.key.lowercased().contains(search.lowercased())
            }
        }
    }
